...
const Booking_participantsTable = () => {
  ...
  const [filters, setFilters] = React.useState([
          ...
          {label: 'Booking Id', title: 'booking_id', number: 'true'},{label: 'Member Id', title: 'member_id', number: 'true'},{label: 'User Id', title: 'user_id', number: 'true'},
  ...
  const modalOpen = useSelector((store) => store.booking_participants.list.modalOpen);
  const rows = useSelector((store) => store.booking_participants.list.rows);
  const idToDelete = useSelector((store) => store.booking_participants.list.idToDelete);
  ...
  const columns = [
...

      {
        ...
        flex: 0.6,
        maxWidth: 80,
        renderCell: (params) => <Actions classes={classes} entity="booking_participants" openModal={openModal} {...params} />,
      }
  ];

  return (
    <div>
      <Widget title={<h4>{humanize('Booking_participants')}</h4>} disableWidgetMenu>
        ...
        <div style={{minHeight: 500, width: "100%", paddingTop: 20, paddingBottom: 20}}>
          <DataGrid
            ...
            disableColumnMenu
            loading={loading}
            onRowClick={(e) => {history.push(`/admin/booking_participants/${e.id}/edit`)}}
            autoHeight
          />
        </div>
