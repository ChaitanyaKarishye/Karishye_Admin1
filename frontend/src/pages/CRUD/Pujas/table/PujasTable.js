...
import * as dataFormat from 'pages/CRUD/Pujas/table/PujasDataFormatters';

import actions from 'actions/pujas/pujasListActions';
...
const PujasTable = () => {
  ...
  const count = useSelector((store) => store.pujas.list.count);
  const modalOpen = useSelector((store) => store.pujas.list.modalOpen);
  const rows = useSelector((store) => store.pujas.list.rows);
  const idToDelete = useSelector((store) => store.pujas.list.idToDelete);
  ...
  const columns = [
...

      {
        ...
        flex: 0.6,
        maxWidth: 80,
        renderCell: (params) => <Actions classes={classes} entity="pujas" openModal={openModal} {...params} />,
      }
  ];

  return (
    <div>
      <Widget title={<h4>{humanize('Pujas')}</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to="/admin/pujas/new">
        ...
        <div style={{minHeight: 500, width: "100%", paddingTop: 20, paddingBottom: 20}}>
          <DataGrid
            ...
            disableColumnMenu
            loading={loading}
            onRowClick={(e) => {history.push(`/admin/pujas/${e.id}/edit`)}}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Pujas'
                : 'http://localhost:8080/api-docs/#/Pujas'
            }
          >
            API documentation for pujas
          </LinkMaterial>
        </div>
      </Widget>
