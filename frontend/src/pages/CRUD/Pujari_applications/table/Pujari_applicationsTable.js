...
const Pujari_applicationsTable = () => {
  ...
  const modalOpen = useSelector(
    (store) => store.pujari_applications.list.modalOpen,
  );
  const rows = useSelector((store) => store.pujari_applications.list.rows);
  const idToDelete = useSelector(
    (store) => store.pujari_applications.list.idToDelete,
  );
  ...
  return (
    <div>
      <Widget
        title={<h4>{humanize('Pujari_applications')}</h4>}
        disableWidgetMenu
      >
        ...
        >
          <DataGrid
            ...
            loading={loading}
            onRowClick={(e) => {
              history.push(`/admin/pujari_applications/${e.id}/edit`);
            }}
            autoHeight
          />
        </div>
