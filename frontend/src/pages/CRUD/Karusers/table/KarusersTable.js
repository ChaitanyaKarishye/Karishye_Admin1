...
import * as dataFormat from 'pages/CRUD/Karusers/table/KarusersDataFormatters';

import actions from 'actions/karusers/karusersListActions';
...
const KarusersTable = () => {
  ...
  const [filters, setFilters] = React.useState([
    ...
    { label: 'Gothram', title: 'gothram' },
    { label: 'Nakshatram', title: 'nakshatram' },
  ...
  const count = useSelector((store) => store.karusers.list.count);
  const modalOpen = useSelector((store) => store.karusers.list.modalOpen);
  const rows = useSelector((store) => store.karusers.list.rows);
  const idToDelete = useSelector((store) => store.karusers.list.idToDelete);
  ...
  return (
    <div>
      <Widget title={<h4>{humanize('Karusers')}</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/karusers/new'>
        ...
        >
          <DataGrid
            ...
            loading={loading}
            onRowClick={(e) => {
              history.push(`/admin/karusers/${e.id}/edit`);
            }}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Karusers'
                : 'http://localhost:8080/api-docs/#/Karusers'
            }
          >
            API documentation for karusers
          </LinkMaterial>
        </div>
      </Widget>
