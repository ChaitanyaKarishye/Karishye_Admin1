...
const Puja_samagri_mappingsTable = () => {
  ...
  const modalOpen = useSelector(
    (store) => store.puja_samagri_mappings.list.modalOpen,
  );
  const rows = useSelector((store) => store.puja_samagri_mappings.list.rows);
  const idToDelete = useSelector(
    (store) => store.puja_samagri_mappings.list.idToDelete,
  );
  ...
  return (
    <div>
      <Widget
        title={<h4>{humanize('Puja_samagri_mappings')}</h4>}
        disableWidgetMenu
      >
        ...
        >
          <DataGrid
            ...
            loading={loading}
            onRowClick={(e) => {
              history.push(`/admin/puja_samagri_mappings/${e.id}/edit`);
            }}
            autoHeight
          />
        </div>
