...
const Booking_samagri_mappingsTable = () => {
  ...
  const [filters, setFilters] = React.useState([
    ...
    { label: 'Samagri Id', title: 'samagri_id', number: 'true' },
    {
  ...
  const count = useSelector(
    (store) => store.booking_samagri_mappings.list.count,
  );
  const modalOpen = useSelector(
    (store) => store.booking_samagri_mappings.list.modalOpen,
  );
  const rows = useSelector((store) => store.booking_samagri_mappings.list.rows);
  const idToDelete = useSelector(
    (store) => store.booking_samagri_mappings.list.idToDelete,
  );
  ...
  const columns = [
...

    {
      ...
      renderCell: (params) => (
        <Actions
          classes={classes}
          entity='booking_samagri_mappings'
          openModal={openModal}
          {...params}
        />
      ),
  ...
  return (
    <div>
      <Widget
        title={<h4>{humanize('Booking_samagri_mappings')}</h4>}
        disableWidgetMenu
      >
        ...
        >
          <DataGrid
            ...
            loading={loading}
            onRowClick={(e) => {
              history.push(`/admin/booking_samagri_mappings/${e.id}/edit`);
            }}
            autoHeight
          />
        </div>
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin +
                  '/api-docs/#/Booking_samagri_mappings'
