...
import * as dataFormat from 'pages/CRUD/Booking_samagri_mappings/table/Booking_samagri_mappingsDataFormatters';

import actions from 'actions/booking_samagri_mappings/booking_samagri_mappingsListActions';
...
const Booking_samagri_mappingsTable = () => {
  ...
  const [filters, setFilters] = React.useState([
          ...
          {label: 'Booking Id', title: 'booking_id', number: 'true'},{label: 'Samagri Id', title: 'samagri_id', number: 'true'},{label: 'No Of Standard Qty', title: 'no_of_standard_qty', number: 'true'},
  ...
  const count = useSelector((store) => store.booking_samagri_mappings.list.count);
  const modalOpen = useSelector((store) => store.booking_samagri_mappings.list.modalOpen);
  const rows = useSelector((store) => store.booking_samagri_mappings.list.rows);
  const idToDelete = useSelector((store) => store.booking_samagri_mappings.list.idToDelete);
  ...
  const columns = [
...

      {
        ...
        flex: 0.6,
        maxWidth: 80,
        renderCell: (params) => <Actions classes={classes} entity="booking_samagri_mappings" openModal={openModal} {...params} />,
      }
  ];

  return (
    <div>
      <Widget title={<h4>{humanize('Booking_samagri_mappings')}</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to="/admin/booking_samagri_mappings/new">
        ...
        <div style={{minHeight: 500, width: "100%", paddingTop: 20, paddingBottom: 20}}>
          <DataGrid
            ...
            disableColumnMenu
            loading={loading}
            onRowClick={(e) => {history.push(`/admin/booking_samagri_mappings/${e.id}/edit`)}}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Booking_samagri_mappings'
                : 'http://localhost:8080/api-docs/#/Booking_samagri_mappings'
            }
          >
            API documentation for booking_samagri_mappings
          </LinkMaterial>
        </div>
      </Widget>
...
export default Booking_samagri_mappingsTable;
