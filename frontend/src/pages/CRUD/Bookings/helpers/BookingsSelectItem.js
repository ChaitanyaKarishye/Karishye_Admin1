...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/bookings/autocomplete`, { params });
  return response.data;
}

const BookingsSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.bookings.hasPermissionToCreate,
});

export default connect(select)(BookingsSelectItem);
