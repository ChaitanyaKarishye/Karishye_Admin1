...
import * as dataFormat from 'pages/CRUD/Bookings/table/BookingsDataFormatters';

import actions from 'actions/bookings/bookingsListActions';
...
const BookingsTable = () => {
  ...
  const [filters, setFilters] = React.useState([
    {label: 'Notes', title: 'notes'},{label: 'Address', title: 'address'},
          {label: 'User Id', title: 'user_id', number: 'true'},{label: 'Pujari Id', title: 'pujari_id', number: 'true'},{label: 'Puja Id', title: 'puja_id', number: 'true'},{label: 'Price', title: 'price', number: 'true'},{label: 'Duration Hrs', title: 'duration_hrs', number: 'true'},{label: 'Base Price', title: 'base_price', number: 'true'},{label: 'Final Price', title: 'final_price', number: 'true'},
  ...
  const count = useSelector((store) => store.bookings.list.count);
  const modalOpen = useSelector((store) => store.bookings.list.modalOpen);
  const rows = useSelector((store) => store.bookings.list.rows);
  const idToDelete = useSelector((store) => store.bookings.list.idToDelete);
  ...
  const columns = [
...

      {
        ...
        flex: 0.6,
        maxWidth: 80,
        renderCell: (params) => <Actions classes={classes} entity="bookings" openModal={openModal} {...params} />,
      }
  ];

  return (
    <div>
      <Widget title={<h4>{humanize('Bookings')}</h4>} disableWidgetMenu>
        ...
        <div style={{minHeight: 500, width: "100%", paddingTop: 20, paddingBottom: 20}}>
          <DataGrid
            ...
            disableColumnMenu
            loading={loading}
            onRowClick={(e) => {history.push(`/admin/bookings/${e.id}/edit`)}}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Bookings'
                : 'http://localhost:8080/api-docs/#/Bookings'
            }
          >
