...
module.exports = class UsersDBApi {
    ...
    const users = await db.users.create(
      {
        ...
        first_time_login: data.data.first_time_login || null,
        forgot_password_token: data.data.forgot_password_token || null,
        remember_token: data.data.remember_token || null,
        forgot_password_token_timestamp:
          data.data.forgot_password_token_timestamp || null,
    ...
    await users.update(
      {
        ...
        first_time_login: data.first_time_login || null,
        forgot_password_token: data.forgot_password_token || null,
        remember_token: data.remember_token || null,
        forgot_password_token_timestamp:
          data.forgot_password_token_timestamp || null,
  ...
  static async findAll(filter, options) {
    ...
    if (filter) {
      ...
      if (filter.forgot_password_token) {
        where = {
          ...where,
          [Op.and]: Utils.ilike(
            'users',
            'forgot_password_token',
            filter.forgot_password_token,
          ),
        };
      }
      ...
      if (filter.email_verified_atRange) {
        const [start, end] = filter.email_verified_atRange;
      ...
      if (filter.first_time_loginRange) {
        const [start, end] = filter.first_time_loginRange;

        if (start !== undefined && start !== null && start !== '') {
          where = {
            ...where,
            first_time_login: {
              ...where.first_time_login,
              [Op.gte]: start,
            },
          };
        }
      ...
      if (filter.forgot_password_token_timestampRange) {
        const [start, end] = filter.forgot_password_token_timestampRange;

        if (start !== undefined && start !== null && start !== '') {
          where = {
            ...where,
            forgot_password_token_timestamp: {
              ...where.forgot_password_token_timestamp,
              [Op.gte]: start,
            },
          };
        }
      ...
      if (filter.is_activeRange) {
        const [start, end] = filter.is_activeRange;
