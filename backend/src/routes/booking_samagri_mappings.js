...
router.post('/', async (req, res) => {
  await Booking_samagri_mappingsService.create(
    req.body.data,
    req.currentUser,
    true,
    req.headers.referer,
...
router.put(
  ...
  wrapAsync(async (req, res) => {
    await Booking_samagri_mappingsService.update(
      req.body.data,
      req.body.id,
      req.currentUser,
    );
...
router.delete(
  '/:id',
  wrapAsync(async (req, res) => {
    await Booking_samagri_mappingsService.remove(
      req.params.id,
      req.currentUser,
    );
    const payload = true;
...
router.get(
  '/',
  wrapAsync(async (req, res) => {
    const payload = await Booking_samagri_mappingsDBApi.findAll(req.query);
