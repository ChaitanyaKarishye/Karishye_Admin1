...
router.post('/', async (req, res) => {
    await BookingsService.create(req.body.data, req.currentUser, true, req.headers.referer);
    const payload = true;
...
router.get('/', wrapAsync(async (req, res) => {
  const payload = await BookingsDBApi.findAll(
    req.query,
  );
...
router.get('/:id', wrapAsync(async (req, res) => {
  const payload = await BookingsDBApi.findBy(
    { id: req.params.id },
  );
